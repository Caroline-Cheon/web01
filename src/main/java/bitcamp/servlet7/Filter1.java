 ...
 *    응답 <--- 리턴  <--- 리턴  <--- 리턴  <--- 리턴 <--- 리턴
 * => 필터 생명 주기
 *    웹 애플리케이션을 시작시킬 때 톰캣 서버가 필터 객체를 자동으로 생성된다.
 *    웹 애플리케이션을 종료시킬 때 톰캣 서버가 필터 객체를 자동으로 소멸된다.
 *    단, web.xml에 등록(애노테이션 설정 포함)된 필터만이 톰캣 서버가 관리한다.
 *    web.xml에 등록되지 않은 필터는 자동 생성되지 않는다.
 * => 필터 꼽기
...
public class Filter1 implements Filter{
  ...
  public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain)
    ...
    // 다음 필터의 doFilter() 또는 서블릿의 service()를 호출하는 명령
