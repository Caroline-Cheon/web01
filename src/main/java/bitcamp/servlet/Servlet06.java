 ...
 *    문제는 ISO-8859-1에는 영어와 라틴문자, 숫자, 특수문자 등
 *    약 200자의 문자에 대해서만 코드 값이 정의되어 있기 때문에
 *    한글이나 한자 일본어 등은 ISO-8859-1로 변환될 수 없다.
 *    그래서 이런 문자들이 물음표(?)로 출력되는 것이다.
 * => 해결책?
 *    출력 스트림을 얻기 전에 어떤 타입의 데이터를 출력할 것인지?
 *    어떤 문자집합으로 변환시켜 출력할 것인지?
 *    지정하면 된다.
 *    response.setContentType("MINE타입;charset=문자집합");
 *    
 * [MINE 표기법] : Multipurpose Internet Mail Extensions
 * - 데이터종류/애플리케이션명
 * - 널리 알려진 MINE 표기
 * 
 */
